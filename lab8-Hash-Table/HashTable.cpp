...
int HashTable::computeHash(const string &s) {
    return s.size() % size;     //stays within array size
}
...
void HashTable::put(const string &s, int score) {
	 list<WordEntry>::iterator itr;     //i value for loop
     if (contains(s)) {     //check hash table if word exists there
        for (itr = hashTable[computeHash(s)].begin(); itr != hashTable[computeHash(s)].end(); ++itr) {  //find word location in list
            if (itr->getWord() == s) {
    ...
    WordEntry newEntry(s, score);       //create and push new entry onto list
...
double HashTable::getAverage(const string &s) {
    ...
    if (contains(s)) {
        for (itr = hashTable[computeHash(s)].begin(); itr != hashTable[computeHash(s)].end(); ++itr) {
            if (itr->getWord() == s) {
                return itr->getAverage();   //call WordEntry getAverage() and return that value
            }
        }
    }
...
bool HashTable::contains(const string &s) {
    list<WordEntry>::iterator itr;
    for (itr = hashTable[computeHash(s)].begin(); itr != hashTable[computeHash(s)].end(); ++itr) {
        if (itr->getWord() == s) {      //word is in hash table
            return true;    
        }
    }

    return false;       //word is not in hash table
}
